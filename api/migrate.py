...
from util.snapshots import get_latest_snapshot_data, snapshot_database_tables
...
def alter_table_statement(table_name, source_metadata):
    ...
    modify_pk = [
        f"MODIFY COLUMN {pk_column.name} int(11) NOT NULL,"
        for pk_column in table.primary_key.columns.values()
    ]
...
def add_trigger(con, table_name, source_metadata):
    table = Table(table_name, source_metadata)
    primaryKeyColNames = [
        f" d.{pk_column.name} = NEW.{pk_column.name} "
        for pk_column in table.primary_key.columns.values()
    ]
    primaryKeyColNames_str = " AND ".join(primaryKeyColNames)
    old_primaryKeyColNames = [
        f" d.{pk_column.name} = OLD.{pk_column.name} "
        for pk_column in table.primary_key.columns.values()
    ]
    old_primaryKeyColNames_str = " AND ".join(old_primaryKeyColNames)
    con.execute(
        text(f"""CREATE TRIGGER {table_name}__ai AFTER INSERT ON {table_name} FOR EACH ROW
    INSERT INTO {table_name}_{history_suffix} SELECT 'insert', NULL, NOW(), d.* 
    FROM {table_name} AS d WHERE {primaryKeyColNames_str};""")
    )
    con.execute(
        text(f"""CREATE TRIGGER {table_name}__au AFTER UPDATE ON {table_name} FOR EACH ROW
    INSERT INTO {table_name}_{history_suffix} SELECT 'update', NULL, NOW(), d.*
    FROM {table_name} AS d WHERE {primaryKeyColNames_str};
                     """)
    )
    con.execute(
        text(f"""CREATE TRIGGER {table_name}__bd BEFORE DELETE ON {table_name} FOR EACH ROW
    ...
    with source_engine.connect() as con:
        for table_name in source_metadata.tables:
            if history_suffix not in table_name:
                con.execute(
                    text(f"DROP table IF EXISTS {table_name}_{history_suffix};")
                )
                print(table_name, history_suffix)
                con.execute(
                    text(
                        f"CREATE TABLE {table_name}_{history_suffix} LIKE {table_name};"
                    )
                )
    ...
    try:
        if localDbConnection.isValid and cloudDbConnection.isValid:
            ...
            if "tables" in data and isinstance(data["tables"], list):
                ...
                output = migrate_table_list(
                    table_names, source_metadata, destination_engine
                )
...
def migrate_all():
    def generate_progress():
        ...
        try:
            if localDbConnection.isValid and cloudDbConnection.isValid:
                ...
                table_row_counts = snapshot_database_tables(
                    source_metadata, source_session, history_suffix
                )
                ...
                with destination_engine.connect() as conn:
                    ...
                    for key, value in table_row_counts.items():
                        table_name = key
                        output[table_name] = migrate_table(
                            table_name, source_metadata, conn, True
                        )
    ...
    return Response(
        stream_with_context(generate_progress()), mimetype="text/event-stream"
    )


def migrate_table_list(
    table_list, source_metadata, destination_engine, shouldLog=False
):
    output = {}
    with destination_engine.connect() as conn:
        conn.execute(text("SET FOREIGN_KEY_CHECKS=0"))
        ...
        for i in range(len(table_list)):
            ...
            output[table_name] = migrate_table(
                table_name, source_metadata, conn, shouldLog
            )
        conn.execute(text("SET FOREIGN_KEY_CHECKS=1"))
        conn.commit()
    return output
        ...
        if localDbConnection.isValid and cloudDbConnection.isValid:
            ...
            with destination_engine.connect() as destination_con:
                with source_engine.connect() as con:
                    for table_name in source_metadata.tables:
                        if history_suffix in table_name:
                            updated_row_count = con.execute(
                                text(
                                    f"select count(*) from  {table_name} where revision_{random_string} = 1;"
                                )
                            )
                            count = updated_row_count.mappings().all()[0]["count(*)"]

                            if count != 0:
                                destination_table_name = table_name.replace(
                                    "_" + history_suffix, ""
                                )
                                # updated_row_count = con.execute(text(f"select count(*) from  {table_name} where revision_{random_string} = 1;"))
                                response["updated_tables"].append(
                                    {destination_table_name: count}
                                )

                                table = Table(table_name, source_metadata)
                                all_col_names = [
                                    c.__getattribute__("name") for c in table.c
                                ]
                                print(all_col_names)
                                primaryKeyColNames = [
                                    pk_column.name
                                ...
                                select_string_t2 = ", ".join(
                                    primaryKeyColNames_no_revision
                                )
                                inner_join_condition = " AND ".join(
                                    [
                                        f" t1.{c} = t2.{c} "
                                        for c in primaryKeyColNames_no_revision
                                    ]
                                )
                                query = f"""
                                SELECT t1.*
                                ...
                                last_updates = list(
                                    con.execute(text(query)).mappings().all()
                                )
                                ...
                                for update in last_updates:
                                    primary_key_condition = " AND ".join(
                                        [
                                            f" {colName} = {value_wrapper(update[colName])} "
                                            for colName in primaryKeyColNames_no_revision
                                        ]
                                    )
                                    insert_values = (
                                        " ( "
                                        + " , ".join([c for c in allColNames_no_suffix])
                                        + " ) "
                                        + "values"
                                        + " ( "
                                        + " , ".join(
                                            [
                                                value_wrapper(update[c])
                                                for c in allColNames_no_suffix
                                            ]
                                        )
                                        + " ) "
                                    )
                                    update_values = " , ".join(
                                        [
                                            f" {c} = {value_wrapper(update[c])} "
                                            for c in allColNames_no_suffix_no_primary
                                        ]
                                    )
                                    ...
                                    if update["action_" + random_string] == "delete":
                                        ...
                                        print(delete_query)
                                    elif update["action_" + random_string] in [
                                        "update",
                                        "insert",
                                    ]:
                                        if (
                                            destination_con.execute(
                                                text(select_query)
                                            ).rowcount
                                            == 0
                                        ):
                                            ...
                                            return make_response(
                                                "not ok, wrong history table structure",
                                                500,
                                            )
                                    else:
                                        return make_response(
                                            "not ok, wrong history table structure", 500
                                        )
